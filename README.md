...
It also includes the application templates, however they are deprecated in 7.14. Using the Red Hat Business Automation Operator is recommended.
...
On the root directory you can also find the two files:
 ...
 - [rhpam-image-streams.yaml](rhpam714-image-streams.yaml): imagestreams definitions, file used to install the product image streams on OpenShift, this files contains the image stream name and the registry the image will be pulled of.
...
To generate the adocs for, example, 7.14.x branch, first, switch the git branch from RHPAM repository then execute the
following command:

```bash
make generate_adocs branch=7.14.x
```

It will use the `7.14.x` branch from *jboss-kie-modules* using the checked out branch on RHPAM repository based from 7.14.x.
