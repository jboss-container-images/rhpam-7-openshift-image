...
This quickstart is intend to be used with the [RHPAM Kie Server](https://github.com/jboss-container-images/rhpam-7-openshift-image/tree/7.2.x/kieserver) image.

## How to use it?

To deploy the Library Process demo you can use the [rhpam72-prod-immutable-kieserver](https://github.com/jboss-container-images/rhpam-7-openshift-image/blob/7.2.x/templates/rhpam72-prod-immutable-kieserver.yaml)
...
```bash
$ oc get template rhpam72-prod-immutable-kieserver -n openshift
Error from server (NotFound): templates.template.openshift.io "rhpam72-prod-immutable-kieserver" not found
```

If you don't have it yet, just install it:

```bash
$ oc create -f https://raw.githubusercontent.com/jboss-container-images/rhpam-7-openshift-image/7.2.x/templates/rhpam72-prod-immutable-kieserver.yaml -n openshift
template.template.openshift.io "rhpam72-prod-immutable-kieserver" created
```

For this template, we also need to install the secrets, which contain the certificates to configure https:
```bash
$ oc create -f https://raw.githubusercontent.com/jboss-container-images/rhpam-7-openshift-image/7.2.x/example-app-secret-template.yaml
...
```bash
$ oc get imagestream rhpam72-kieserver-openshift -n openshift
Error from server (NotFound): imagestreams.image.openshift.io "rhpam72-kieserver-openshift" not found

```
 If the `rhpam72-kieserver-openshift` is not found, install it under the 'openshift' namespace:
 ```bash
$ oc create -f https://raw.githubusercontent.com/jboss-container-images/rhpam-7-openshift-image/master/rhpam72-image-streams.yaml -n openshift
```
...
```bash
$ oc new-app rhpam72-prod-immutable-kieserver \
...
--> Deploying template "openshift/rhpam72-prod-immutable-kieserver" to project rhpam-kieserver

     Red Hat Process Automation Manager 7.2 immutable production environment
     ---------
     ...
     * With parameters:
        ...
        * KIE Server ImageStream Name=rhpam72-kieserver-openshift
        * ImageStream Tag=1.1
