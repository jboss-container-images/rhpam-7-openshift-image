...
This quickstart is intend to be used with the [RHPAM Kie Server](https://github.com/jboss-container-images/rhpam-7-openshift-image/tree/7.1.x/kieserver) image.

## How to use it?

To deploy the Library Process demo you can use the [rhpam71-prod-immutable-kieserver](https://github.com/jboss-container-images/rhpam-7-openshift-image/blob/7.1.x/templates/rhpam71-prod-immutable-kieserver.yaml)
...
```bash
$ oc get template rhpam71-prod-immutable-kieserver -n openshift
Error from server (NotFound): templates.template.openshift.io "rhpam71-prod-immutable-kieserver" not found
```
If you don't have it yet, just install it:

```bash
oc create -f https://raw.githubusercontent.com/jboss-container-images/rhpam-7-openshift-image/7.1.x/templates/rhpam71-prod-immutable-kieserver.yaml -n openshift
...
```bash
$ oc create -f https://raw.githubusercontent.com/jboss-container-images/rhpam-7-openshift-image/7.1.x/example-app-secret-template.yaml
...
```bash
$ oc get imagestream rhpam71-kieserver-openshift -n openshift
Error from server (NotFound): imagestreams.image.openshift.io "rhpam71-kieserver-openshift" not found
```

If the `rhpam71-kieserver-openshift` is not found, install it under the 'openshift' namespace:
 ```bash
$ oc create -f https://raw.githubusercontent.com/jboss-container-images/rhpam-7-openshift-image/7.1.x/rhpam71-image-streams.yaml -n openshift
```
...
--> Deploying template "openshift/rhpam71-prod-immutable-kieserver" to project rhpam-kieserver

     Red Hat Process Automation Manager 7.1 immutable production environment
     ---------
     ...
     * With parameters:
        ...
        * KIE Admin Password=pvaHMY7! # generated
        * KIE Server User=executionUser
        * KIE Server Password=EtNugU3! # generated
        * ImageStream Namespace=openshift
        * KIE Server ImageStream Name=rhpam71-kieserver-openshift
        * ImageStream Tag=1.1
        ...
        * KIE Server PostgreSQL Database Password=iIUdPs3! # generated
        ...
        * Github Webhook Secret=tUHk3iJi # generated
        * Generic Webhook Secret=vBVwVjHS # generated
        * Maven mirror URL=
        * Maven repository ID=
...
--> Success
    Access your application via route 'myapp-kieserver-rhpam-kieserver.severinocloud.com' 
    Access your application via route 'secure-myapp-kieserver-rhpam-kieserver.severinocloud.com' 
    Build scheduled, use 'oc logs -f b
```
